...
double inf_norm_vector(double* vec, int n) {
    double max_val = 0.0;
    for (int i = 0; i < n; i++) {
...
double inf_norm_matrix(double* mat, int n) {
    double max_row_sum = 0.0;
    for (int i = 0; i < n; i++) {
        double row_sum = 0.0;
        for (int j = 0; j < n; j++) {
            row_sum += fabs(mat[i * n + j]);
        }
...
double calculate_error_E1(double* A, double* L, double* U, int* P, int n) {
    ...
    double norm = inf_norm_matrix(diff, n);
    free(PA);
    free(LU);
    free(diff);
    return norm;
...
double calculate_error_E2(double* A, double* x, double* b, int n) {
    ...
    double norm = inf_norm_vector(diff, n);
    free(Ax);
    free(diff);
    return norm;
}

double calculate_error_E3(double* x, double* x_true, int n) {
    double* diff = (double*)malloc(n * sizeof(double));
    for (int i = 0; i < n; i++) {
        diff[i] = x[i] - x_true[i];
    }
...
// ---------------------- System Solving with Iterative Refinement ----------------------

void solve_system(double* A, double* L, double* U, int* P, double* b, double* x, int n, int refinement_steps) {
    ...
    for (int i = n - 1; i >= 0; i--) {
        ...
        if (fabs(diag) < DBL_EPSILON)
            diag = (diag >= 0 ? DBL_EPSILON : -DBL_EPSILON);
        x[i] /= diag;
    }
    ...
    for (int step = 0; step < refinement_steps; step++) {
        ...
        for (int i = 0; i < n; i++) {
            r[i] = b[i] - Ax[i];
            if (fabs(r[i]) > residual_norm)
                residual_norm = fabs(r[i]);
        }
